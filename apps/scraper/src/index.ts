...
import { syncCustomerKana } from './scrapers/customer-kana.js';

import { supabase, clearAllSyncLocks } from './lib/supabase.js';
...
if (rangeDays > 0) {
  ...
  // Google Calendar 同期（排他制御付き）
  async function syncGoogle() {
    if (isGoogleSyncing) {
      console.log('[skip] Google Calendar 同期がまだ実行中です');
      return;
    }
    ...
    try {
      ...
      console.error('Google Calendar 同期エラー:', err);
    } finally {
      isGoogleSyncing = false;
    }
  }

  // まず即時実行：60日分同期 + カレンダー同期
  syncRange(60);
  syncGoogle();

  // 15分ごとに今日分を同期（新規予約をキャッチ）+ カレンダー同期
  ...
  cron.schedule('0 6 * * *', () => {
    console.log('\n[cron] かな辞書同期を開始...');
    syncCustomerKana().catch(err => console.error('かな辞書同期エラー:', err));
  }, {
    timezone: 'Asia/Tokyo',
  });

  console.log('\n定期同期スケジューラーを開始しました');
  console.log('- 15分ごと: 当日分');
  console.log('- 2時間ごと: 今後60日分');
  console.log('- 毎日6時: かな辞書同期');
